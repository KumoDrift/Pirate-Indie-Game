...
export function createApp(root: HTMLElement): void {
  ...
  debugControls.innerHTML = `
    <div class="panel-group">
      ...
      <label class="toggle-row"><input id="show-hitboxes" type="checkbox" /> Hitboxes</label>
      <label class="toggle-row"><input id="show-attackboxes" type="checkbox" /> Attackboxes</label>
      
      <div class="toggle-row" style="margin-top: 10px; display: flex; flex-direction: column; gap: 4px;">
        <span style="font-size: 11px; color: #94a3b8; font-weight: bold; text-transform: uppercase; letter-spacing: 0.5px;">Active BGM</span>
        <select id="bgm-select" style="background: #1e293b; color: #f8fafc; border: 1px solid #475569; padding: 4px 8px; border-radius: 4px; font-family: monospace; font-size: 12px; outline: none; cursor: pointer; width: 100%;">
          <option value="bgm-battle">Action Battle Theme</option>
        </select>
      </div>

      <div class="toggle-row" style="margin-top: 10px; display: flex; flex-direction: column; gap: 4px;">
        <span style="font-size: 11px; color: #94a3b8; font-weight: bold; text-transform: uppercase; letter-spacing: 0.5px;">Active Background</span>
        <select id="bg-select" style="background: #1e293b; color: #f8fafc; border: 1px solid #475569; padding: 4px 8px; border-radius: 4px; font-family: monospace; font-size: 12px; outline: none; cursor: pointer; width: 100%;">
          <option value="bg-port-town">Port Town Dock</option>
          <option value="bg-pirate-ship-deck">Pirate Ship Deck</option>
          <option value="bg-skeleton-island">Skeleton Island</option>
        </select>
      </div>
    </div>
  ...
  const showVisual = debugControls.querySelector<HTMLInputElement>('#show-visual-bounds');
  const showHitbox = debugControls.querySelector<HTMLInputElement>('#show-hitboxes');
  const showAttackbox = debugControls.querySelector<HTMLInputElement>('#show-attackboxes');
  const bgmSelect = debugControls.querySelector<HTMLSelectElement>('#bgm-select');
  const bgSelect = debugControls.querySelector<HTMLSelectElement>('#bg-select');
  ...
  showVisual.addEventListener('change', () => {
    debugStore.patchState({ showVisualBounds: showVisual.checked });
  });

  showHitbox.addEventListener('change', () => {
    debugStore.patchState({ showHitboxes: showHitbox.checked });
  });

  showAttackbox.addEventListener('change', () => {
    debugStore.patchState({ showAttackboxes: showAttackbox.checked });
  });

  bgmSelect.addEventListener('change', () => {
    debugStore.patchState({ activeBgm: bgmSelect.value });
  });

  bgSelect.addEventListener('change', () => {
    debugStore.patchState({ activeBg: bgSelect.value });
  });
  ...
  debugStore.subscribe((state) => {
    ...
    showVisual.checked = state.showVisualBounds;
    showHitbox.checked = state.showHitboxes;
    showAttackbox.checked = state.showAttackboxes;
    bgmSelect.value = state.activeBgm;
    bgSelect.value = state.activeBg;
